...
department, photo uploads) save to the browser's `localStorage` only.
Use the **Export employees.js** / **Export departments.js** buttons on
the Admin/Departments pages to download an updated data file, then
commit it to `src/data/` so the change ships to everyone on the next
deploy.
...
thing every host needs to be configured for is described next.
